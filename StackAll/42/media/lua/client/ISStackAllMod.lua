...
--for i=1,102 do local item = instanceItem("Base.Thread"); item:setUsedDelta(0.1); inv:addItem(item) end
--for i=1,102 do local item = instanceItem("Base.PillsVitamins"); item:setUsedDelta(0.2); inv:addItem(item) end
--for i=1,102 do local item = instanceItem("Base.Soap2"); item:setUsedDelta(0.1); inv:addItem(item) end
...
local onConsolidate = ISInventoryPaneContextMenu.onConsolidate or function(drainable, intoItem, player)
	ISTimedActionQueue.add(ISConsolidateDrainable:new(player, drainable, intoItem, nil));
end
...
local function getSoapAlgorithm(inv,typ)
	...
	inv:getSomeTypeEval(typ,function(item)
		local used_delta = item:getCurrentUsesFloat()
...
function ISStackAllMod:perform() print('stack perform')
	...
	for typ,some_item in pairs(self.arr) do --print("typ = ",typ)
		...
		local max_delta = -9999
		local min, max
		if (typ == "Base.Soap2" and STAR_MODS.StackAll.SANDBOX.MergeableSoap) then
		...
		if min and max and min ~= max then
			...
			onConsolidate(min, max, self.character) --pour
			STAR_MODS.StackAll.is_drop_time_to_zero = true
			--ISTimedActionQueue.addAfter(self, ISStackAllMod:new(self.character, self.arr)) --and check again
