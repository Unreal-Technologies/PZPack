...
local function InGame()
	local inst = MainScreen.instance
	return inst and inst.inGame
end
...
------------Bind a key--------------
local Options = PZAPI.ModOptions:create("ReadWalkingStar", "Read While Walking")

local KEY_READ = Options:addKeyBind( -- (id, name, value, tooltip)
  "resume_read",
  getText("UI_options_ReadWalkingStar_keyBind_Read"),
  Keyboard.KEY_R,
  getText("UI_options_ReadWalkingStar_keyBind_Read_tooltip")
);
...
local function TryResumeReadingBook()
	...
	local is_key_R = KEY_READ.key == Keyboard.KEY_R
