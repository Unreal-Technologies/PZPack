...
local function adjustStats(character, tracklevel)
	...
	--LEVEL
	Level = ((tonumber(PlayerMusicLevel))/1.5)
		...
		if Failstate == false then
			if self.learnedSong then
				...
				elseif (#self.PriorityToLearnMid > 0) and (playerlevel > 2) then
					randomDiceTarget = (201 - (playerlevel+trait))
...
function PlayInstrumentTraining:start()
	...
	if self.instrumentType == "Trumpet" then
		self.instrumentSounds = require("TimedActions/PlayTrumpetTracks")
		self.learnedTracksData = self.character:getModData().TrumpetLearnedTracks
	elseif self.instrumentType == "GuitarAcoustic" then
		self.instrumentSounds = require("TimedActions/PlayGuitarAcousticTracks")
		self.learnedTracksData = self.character:getModData().GuitarALearnedTracks
	elseif self.instrumentType == "Banjo" then
		self.instrumentSounds = require("TimedActions/PlayBanjoTracks")
		self.learnedTracksData = self.character:getModData().BanjoLearnedTracks
	elseif self.instrumentType == "Keytar" then
		self.instrumentSounds = require("TimedActions/PlayKeytarTracks")
		self.learnedTracksData = self.character:getModData().KeytarLearnedTracks
	elseif self.instrumentType == "Saxophone" then
		self.instrumentSounds = require("TimedActions/PlaySaxophoneTracks")
		self.learnedTracksData = self.character:getModData().SaxophoneLearnedTracks
	elseif self.instrumentType == "GuitarElectricBass" then
		self.instrumentSounds = require("TimedActions/PlayGuitarElectricBassTracks")
		self.learnedTracksData = self.character:getModData().GuitarEBLearnedTracks
	elseif self.instrumentType == "GuitarElectric" then
		self.instrumentSounds = require("TimedActions/PlayGuitarElectricTracks")
		self.learnedTracksData = self.character:getModData().GuitarELearnedTracks
	elseif self.instrumentType == "Flute" then
		self.instrumentSounds = require("TimedActions/PlayFluteTracks")
		self.learnedTracksData = self.character:getModData().FluteLearnedTracks
	elseif self.instrumentType == "Harmonica" then
		self.instrumentSounds = require("TimedActions/PlayHarmonicaTracks")
		self.learnedTracksData = self.character:getModData().HarmonicaLearnedTracks
	end
	...
	if self.level > 1 and self.learnedTracksData then
		...
		if #self.learnedTracksData > 0 then
			for k,v in pairs(self.learnedTracksData) do
				if v.level > 1 and v.level <= self.level and (self.level < 3 or (v.level+2 >= self.level)) then
					table.insert(self.AvailableInstrumentTracks, v)
				end
			end
		end
	...
	if #self.AvailableInstrumentTracks <= 3 then
		...
		if self.level > 1 and self.learnedTracksData then
			if #self.learnedTracksData > 0 then
			for k,v in pairs(self.learnedTracksData) do
				if v.level > 1 and v.level <= self.level and (self.level >= 3 and (v.level+2 < self.level)) then
					table.insert(self.AvailableInstrumentTracks, v)
				end
			end
		end
...
function PlayInstrumentTraining:new(character, Item, Type, level)
    local o = {}
    setmetatable(o, self)
    self.__index = self
