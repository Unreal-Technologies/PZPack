...
function ISNapkinWashMyself:isValid()
	return self.character:getInventory():contains(self.item) and ISWashYourself.GetRequiredWater(self.character) > 0 and self.item:getCurrentUsesFloat() > 0;
end
...
function ISNapkinWashMyself:perform()
	...
	for i=1,BloodBodyPartType.MAX:index() do
		...
		if self:washPart(visual, part) then
			if self.item and self.item:getCurrentUsesFloat() > 0 then
				self.item:Use()
			else
				break
			end
