	...
	if SiloGenerator:getPipedFuelAmount() < 500 then
		...
		if playerItems ~= nil then
			...
			menu:addSubMenu(contextMenu3, subContext3)
				...
				subContext3:addOption(FA.translation.addallfuel,
					nil,	
				  	FASiloGenerator.onUseSiloGenerator,
				  	getSpecificPlayer(player),
				  	SiloGenerator,
			...
			for i=0, playerItems:size()-1 do
				...
				if item:hasTag("Petrol") then
					subContext3:addOption(item:getDisplayName(),
				  		nil,	
				  		FASiloGenerator.onUseSiloGenerator,
				  		getSpecificPlayer(player),
				  		SiloGenerator,
...
FASiloGenerator.onUseSiloGenerator = function(junk, player, SiloGenerator, NewSiloGenerator, fuelCan, MyChoice)
	...
	if MyChoice == "addfuel" then
		if fuelCan ~= nil then
			...
			ISTimedActionQueue.add(SiloGeneratorAddFuel:new(player, SiloGenerator, fuelCan, (fuelCan:getUsedDelta()/fuelCan:getUseDelta()*25)))
		else
			player:Say(FA.translation.sayneedcanoffuel)
		end
		return
	elseif MyChoice == "addallfuel" then
		...
		if playerItems ~= nil then
			for i=0, playerItems:size()-1 do
        			local item = playerItems:get(i)
				if item:hasTag("Petrol") then
					ISTimedActionQueue.add(SiloGeneratorAddFuel:new(player, SiloGenerator, item, (item:getUsedDelta()/item:getUseDelta()*25)))
					foundFuel = true
				end
			end
		elseif foundFuel == false then
			player:Say(FA.translation.sayneedcanoffuel)
		end
		return
...
FA.checkSiloGeneratorStatus = function()
	...
	if #FA.SiloGeneratorList > 0 and removedFromSquare ~= nil then
