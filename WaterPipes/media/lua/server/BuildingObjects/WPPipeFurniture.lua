WPPipeFurniture = ISBuildingObject:derive("WPSimpleFurniture");

--************************************************************************--
--** WPSimpleFurniture:new
--**
--************************************************************************--
function WPPipeFurniture:create(x, y, z, north, sprite)
...
function WPPipeFurniture:removeFromGround(square)
...
function WPPipeFurniture:new(name, sprite, northSprite)
	local o = {};
	setmetatable(o, self);
	self.__index = self;
	o:init();
...
function WPPipeFurniture:getHealth()
	return 100 + buildUtil.getWoodHealth(self);
end

function WPPipeFurniture:isValid(square)
	if not ISBuildingObject.isValid(self, square) then return false; end
	if not buildUtil.canBePlace(self, square) then return false end
    if PipeIsOnSquare(square) then return false end
...
function WPPipeFurniture:render(x, y, z, square)
