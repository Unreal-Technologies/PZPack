...
local function tooltipRequiredPPE(player)
    ...
    if not (hasDustMask and hasSafetyGoggles) and not hasGasMask and not hasBioMask then
        notAvailable = true
    end
...
function PaintVehicle.addOptionToMenuOutsideVehicle(player, context, vehicle)
    ...
    for _, v in pairs(PaintVehicleConfig.VEHICLES) do
        if v.id == vehicle:getScriptName() then
            paintRequired = v.paint
            break
        end
    end
    ...
    if not data.isSanded and not data.isPrimed then
        addSandOption(player, context, vehicle, paintRequired)
    elseif data.isSanded and not data.isPrimed then
        addPrimeOption(player, context, vehicle, paintRequired)
    elseif data.isSanded and data.isPrimed then
        addPaintOption(player, context, vehicle, paintRequired)
    end
end

if not PaintVehicle.originalMenuOutsideVehicle then
    PaintVehicle.originalMenuOutsideVehicle = ISVehicleMenu.FillMenuOutsideVehicle
end
function ISVehicleMenu.FillMenuOutsideVehicle(player, context, vehicle, test)
    PaintVehicle.originalMenuOutsideVehicle(player, context, vehicle, test)
    PaintVehicle.addOptionToMenuOutsideVehicle(getSpecificPlayer(player), context, vehicle)
end